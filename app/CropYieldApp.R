        ...
        fluidRow(
          column(
            ...
            sliderInput("yearRange", "Select Year Range:",
                        ...
                        value = c(min(as.integer(format(combined_data$ds, 
                                                        "%Y"))),
                                  max(as.integer(format(combined_data$ds, 
                                                        "%Y")))),
                        step = 1, sep = "")
          ),
          column(
...
server <- function(input, output) {
  ...
  output$forecastPlot <- renderPlot({
    ...
    if (input$showCI) {
      gg <- gg + geom_ribbon(aes(ymin = yhat_lower, ymax = yhat_upper), 
    ...
    gg + scale_color_manual(values = c("Forecast" = "blue", 
                                       "Actual" = "black")) +
  ...
  output$residualsPlot <- renderPlot({
    ...
    ggplot(df, aes(x = ds, y = residual)) +
      ...
      labs(title = "Residuals Over Time", x = "Date", 
